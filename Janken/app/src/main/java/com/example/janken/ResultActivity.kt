...
class ResultActivity : AppCompatActivity() {
    ...
    private fun getHand(): Int{
        ...
        val pref = PreferenceManager.getDefaultSharedPreferences(this)
        val gameCount = pref.getInt("GAME一COUNT", 0)
        val winningStreakCount = pref.getInt("WINNING_STREAK_COUNT", 0)
        val lastMyHand = pref.getInt("LAST_MY_HAND", 0)
        val lastComHand = pref.getInt("LAST_C0M_HAND", 0)
        val beforeLastComHand = pref.getInt("BEF0RE_LAST_C0M_HAND",0)
        val gameResult= pref.getInt("GAME_RESULT", -1)

        if (gameCount ==1) {
            if (gameResult ==2) {
                //前回の勝負が1回目で、コンピュータが勝った場合、
                // コンピュータは次に出す手を変える
                while (lastComHand == hand) {
                    hand = (Math.random() * 3).toInt()
                }
            }else if(gameResult ==1){
                //前回の勝負がユ回目で、コンピュータが負けた場合
                // 相手の出した手に勝つ手を出す
